import { DAALDER_API_BASE_URL } from '../constants'
...
const apiMethod = (method: string) => async (uri: string) => {
  const response = await fetch(`${DAALDER_API_BASE_URL}${uri}`, { method })
  return handleResponse(response)
}

const apiMethodWithPayload = (method: string) => async (uri: string, data: {}) => {
  const response = await fetch(`${DAALDER_API_BASE_URL}${uri}`, {
    method,
    headers: {
