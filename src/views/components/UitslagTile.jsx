...
const UitslagTile = ({ date, aanmelding, sollicitatie, markt, toewijzing, afwijzing }) => {
    const aangemeld =
    ...
    return (
        <div className="col-1-2 UitslagTile">
            <div className="UitslagTile__datum">
                ...
                {!marktGesloten ? (
                    <h4
                        className={`UitslagTile__datum__aanwezigheid ${
                            aangemeld ? `UitslagTile__datum__aanwezigheid--aangemeld` : null
                        }`}
                    >
                        {aangemeld ? 'Aangemeld' : 'Niet aangemeld'}
                    </h4>
                ) : null}
                {marktGesloten ? <h4 className="UitslagTile__datum__aanwezigheid">Geen markt</h4> : null}
            </div>
            ...
            {!marktGesloten && afwijzing && markt.kiesJeKraamFase === 'live' ? (
                <AlertLine
                    ...
                    titleSmall={true}
                    message={`${
                        afwijzing.reasonCode
                            ? printAfwijzingReason(afwijzing.reasonCode)
                            : 'Het is niet gelukt u in te delen.'
                    }`}
                    inline={true}
                />
            ) : null}
