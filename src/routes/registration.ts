...
module.exports = () => {
    ...
    router.route('/activeren').get(activationPage).post(handleActivation);
    router.route('/registreren').get(registrationPage).post(handleRegistration);
...
const handleActivation = (req, res) => {
    ...
    Promise.all([checkActivationCode(username, code), userExists(username)])
...
const handleRegistration = (req, res) => {
    ...
    if (!req.session.activation) {
        ...
        return res.redirect(
            `/registreren${stringify({ email, error: publicErrors.NON_MATCHING_PASSWORDS }, { addQueryPrefix: true })}`,
        );
    }
